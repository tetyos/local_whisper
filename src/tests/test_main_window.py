...
from local_whisper.ui.main_window import MainWindow
from local_whisper.ui.model_selector_view import ModelCard, ModelSelectorView
from local_whisper.ui.main_view import MainView
...
def mock_transcriber(monkeypatch):
    ...
    monkeypatch.setattr("local_whisper.ui.model_selector_view.Transcriber.get_available_models", lambda: mock_models)
    monkeypatch.setattr("local_whisper.ui.model_selector_view.Transcriber.is_model_downloaded", lambda x: x == "tiny")
...
class TestMainWindowInit:
    ...
    def test_has_main_view(self, main_window):
        """Test that main view exists."""
        assert main_window.main_view is not None
        assert isinstance(main_window.main_view, MainView)
    
    def test_has_model_selector_view(self, main_window):
        """Test that model selector view exists."""
        assert main_window.model_selector_view is not None
        assert isinstance(main_window.model_selector_view, ModelSelectorView)
    
    def test_main_view_has_title_label(self, main_window):
        """Test that title label exists in main view."""
        assert main_window.main_view.title_label is not None
        assert "local-whisper" in main_window.main_view.title_label.text()
    
    def test_main_view_has_status_label(self, main_window):
        """Test that status label exists in main view."""
        assert main_window.main_view.status_label is not None
    
    def test_main_view_has_hotkey_label(self, main_window):
        """Test that hotkey label exists in main view."""
        assert main_window.main_view.hotkey_label is not None
        assert "Ctrl" in main_window.main_view.hotkey_label.text()
        assert "Space" in main_window.main_view.hotkey_label.text()
    
    def test_main_view_has_models_button(self, main_window):
        """Test that models button exists in main view."""
        assert main_window.main_view.models_button is not None
...
class TestMainWindowStatus:
    ...
    def test_set_status_updates_label(self, main_window):
        ...
        assert main_window.main_view.status_label.text() == "Testing status"
    ...
    def test_set_status_recording_style(self, main_window):
        ...
        style = main_window.main_view.status_label.styleSheet()
    ...
    def test_set_status_hides_progress_bar(self, main_window):
        ...
        main_window.main_view.transcription_progress_bar.setVisible(True)
        
        main_window.set_status("Ready", is_transcribing=False)
        
        assert main_window.main_view.transcription_progress_bar.isVisible() is False
...
class TestMainWindowModel:
    ...
    def test_set_current_model_updates_label(self, main_window):
        ...
        assert main_window.main_view.model_display_label.text() == "OpenAI Whisper Base"
    ...
    def test_set_current_model_empty_shows_no_model(self, main_window):
        ...
        assert "No model" in main_window.main_view.model_display_label.text()
...
class TestMainWindowTranscriptionProgress:
    ...
    def test_update_transcription_progress_shows_bar(self, main_window):
        ...
        assert main_window.main_view.transcription_progress_bar.isVisible()
        assert main_window.main_view.eta_label.isVisible()
    ...
    def test_update_transcription_progress_value(self, main_window):
        ...
        assert main_window.main_view.transcription_progress_bar.value() == 75
    ...
    def test_update_transcription_progress_eta_format(self, main_window):
        ...
        eta_text = main_window.main_view.eta_label.text()
...
class TestMainWindowModelSelector:
    ...
    def test_models_button_switches_view(self, main_window, qtbot):
        ...
        qtbot.mouseClick(main_window.main_view.models_button, Qt.MouseButton.LeftButton)
    ...
    def test_back_button_returns_to_main(self, main_window, qtbot):
        ...
        qtbot.mouseClick(main_window.model_selector_view.back_button, Qt.MouseButton.LeftButton)
        
        assert main_window.stacked_widget.currentIndex() == 0
    
    def test_model_cards_created(self, main_window, mock_transcriber):
        """Test that model cards are created for each model."""
        assert len(main_window.model_selector_view._model_cards) == len(mock_transcriber)
    
    def test_use_button_disabled_initially(self, main_window):
        """Test that use button is disabled when no selection."""
        main_window.model_selector_view._selected_model = ""
        main_window.model_selector_view._update_use_button_state()
        
        assert main_window.model_selector_view.use_button.isEnabled() is False
...
class TestMainWindowDownloading:
    ...
    def test_set_downloading_disables_buttons(self, main_window):
        ...
        assert main_window.model_selector_view._is_downloading is True
        assert main_window.model_selector_view.back_button.isEnabled() is False
    ...
    def test_set_downloading_false_enables_buttons(self, main_window):
        ...
        assert main_window.model_selector_view._is_downloading is False
        assert main_window.model_selector_view.back_button.isEnabled() is True
    ...
    def test_update_download_progress(self, main_window):
        ...
        assert main_window.model_selector_view.global_progress_bar.isVisible()
        assert main_window.model_selector_view.global_progress_bar.value() == 50
    ...
    def test_download_complete_hides_progress(self, main_window, mock_transcriber):
        ...
        main_window.model_selector_view.global_progress_bar.setVisible(True)
        
        main_window.download_complete("tiny")
        
        assert main_window.model_selector_view.global_progress_bar.isVisible() is False
...
class TestMainWindowSignals:
    ...
    def test_model_selected_signal(self, main_window, qtbot, mock_transcriber):
        ...
        main_window.model_selector_view._selected_model = "tiny"
        main_window.model_selector_view._model_cards["tiny"]._is_downloaded = True
        
        with qtbot.waitSignal(main_window.model_selected, timeout=1000) as blocker:
            main_window.model_selector_view._on_use_clicked()
    ...
    def test_download_requested_signal(self, main_window, qtbot, mock_transcriber):
        ...
        with qtbot.waitSignal(main_window.download_requested, timeout=1000) as blocker:
            main_window.model_selector_view._on_download_requested("base")
...
class TestMainWindowLoadingState:
    ...
    def test_set_loading_updates_status(self, main_window):
        ...
        assert main_window.main_view.status_label.text() == "Loading model..."
...
class TestMainWindowModelsButtonState:
    ...
    def test_set_models_button_enabled(self, main_window):
        ...
        assert main_window.main_view.models_button.isEnabled()
    ...
    def test_set_models_button_disabled(self, main_window):
        ...
        assert main_window.main_view.models_button.isEnabled() is False
...
class TestModelCard:
    ...
    def model_card(self, qtbot, monkeypatch):
        ...
        monkeypatch.setattr("local_whisper.ui.model_selector_view.Transcriber.is_model_downloaded", lambda x: False)
